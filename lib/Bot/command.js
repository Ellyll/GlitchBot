...
function processCommand(bot, botCommand, channel, botName, shutdown) {
    ...
    switch (botCommand.name) {
        ...
        case 'RACE':
            ...
            bot.say(channel, botCommand.from + ' raced a ' + raced.cubimal + ' and it went ' + raced.distance + ' planks!');
            break;

        case 'CUBIMAL':
            var cubimal = glitch.cubimals.getRandomCubimal();
            bot.say(channel, botCommand.from + ' opens a cubimal box....and got a ' + cubimal + '!');
            break;
